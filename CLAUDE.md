...
- Write the changelog only when the owner asks for a release, and by hand — no script touches it. Collect what shipped since the last tag into short lines, put them under a `## vX.Y.Z — YYYY-MM-DD` heading (keep an empty `## Unreleased` above), commit that, then run `make release vX.Y.Z` (or `v=X.Y.Z`) to move the version and tag. Push the tag, then publish a GitHub release for it (`gh release create vX.Y.Z --notes-from-tag`) — that release, not the tag, is what builds and pushes the image and tells Komodo to redeploy. Do not add entries commit-by-commit.
