...
| Variable | Default | What it does |
|---|---|---|
| `BLUNDERBASE_ANALYSIS_CONCURRENCY` | unset | Pins **Queue processes** — engine processes running at once, shared by every pass, search and board — from outside. Unset, the number is the setting on [Machines](runners.md#how-much-at-once), which defaults to the machine's cores minus two. Set, that field is read-only. This caps CPU, not connections |
