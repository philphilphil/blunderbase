...
# collide with the last one they kept. The site's download buttons link these names
# directly: scripts/site.sh writes the version from pyproject.toml into the page, and the
# push below is what rebuilds blunderbase.org, so the two names here and the two URLs
# there have to agree. The name is built from the tag, and the file it is built from is
# found by version, so a stale build from the last release cannot ride along under this
# release's name.
find_installer() {
...
find_installers() {
	...
	if [ -z "$mac" ] || [ -z "$win" ]; then
		[ -n "${BB_SKIP_DESKTOP:-}" ] || die "desktop/dist has no $version installers for both platforms — run \`make desktop\` first, or BB_SKIP_DESKTOP=1 to release without them (the site's download buttons will point at files that do not exist)"
	fi
}
