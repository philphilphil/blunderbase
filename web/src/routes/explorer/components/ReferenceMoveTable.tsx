 ...
 *
 * The columns are the four facts a reference book has: how often the move is played
 ...
 * which would claim a result was good news for a reader who is not in the game), and the
 * average rating of the players who chose it, which is how a move that is popular and a
 * move that is respected are told apart.
 *
...
/**
 * 240px of fixed columns plus the gaps, the padding and a readable minimum for the bar.
 * Fewer columns than the owner's table, so it needs less room before it has to scroll
 * sideways — but it still scrolls rather than crushing the bar to a smear on a phone.
 */
const MIN_TABLE = 'max-md:min-w-[30rem]'
