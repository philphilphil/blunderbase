...
export function statusLabel(runner: RunnerResponse): RunnerStatusLabel {
  if (!runner.connected) return { label: 'not connected', tone: 'away' }
  if (runner.transport === 'poll') {
    return { label: 'connected · polling — queue only', tone: 'degraded' }
  }
  return { label: 'connected · websocket', tone: 'connected' }
}
...
export function slotLabel(runner: RunnerResponse): string {
  ...
  return `${used}/${runner.slots} slots`
}
