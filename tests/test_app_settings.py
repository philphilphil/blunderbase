...
def test_nothing_is_configured_until_somebody_configures_it(session: Session) -> None:
    ...
    assert app_settings.get_analysis_nodes(session) == 500_000
    assert app_settings.get_analysis_multipv(session) == 2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (app_settings.ANALYSIS_NODES, 0, app_settings.MIN_NODES),
        (app_settings.ANALYSIS_NODES, -5, app_settings.MIN_NODES),
        # Five lines, the most the Analyse dialog offers.
        (app_settings.ANALYSIS_MULTIPV, 99, 5),
        (app_settings.ANALYSIS_MULTIPV, 0, app_settings.MIN_MULTIPV),
        ...
        (app_settings.MAIA_ON_ANALYSIS, 7, app_settings.FLAG_ON),
        (app_settings.MAIA_ON_ANALYSIS, -1, app_settings.FLAG_OFF),
    ],
)
...
def test_clearing_one_removes_the_row_rather_than_writing_a_null(session: Session) -> None:
    ...
    app_settings.set_value(session, app_settings.ANALYSIS_NODES, 50_000)

    assert app_settings.set_value(session, app_settings.ANALYSIS_NODES, None) is None
    assert app_settings.stored(session, app_settings.ANALYSIS_NODES) is None
    assert app_settings.get_analysis_nodes(session) == app_settings.ANALYSIS_NODES_DEFAULT
...
def test_a_row_edited_by_hand_cannot_break_a_caller(session: Session) -> None:
    ...
    session.add(AppSetting(key=app_settings.ANALYSIS_NODES, value=True))
    session.commit()
...
def test_a_whole_replacement_clears_what_it_leaves_out(session: Session) -> None:
    app_settings.set_value(session, app_settings.ANALYSIS_MULTIPV, 4)

    assert app_settings.replace(session, {app_settings.ANALYSIS_NODES: 1000}) == {
        **NOTHING_SET,
        app_settings.ANALYSIS_NODES: 1000,
    }
    assert app_settings.get_analysis_multipv(session) == app_settings.ANALYSIS_MULTIPV_DEFAULT
...
def test_a_refused_set_of_thresholds_writes_nothing(session: Session) -> None:
    app_settings.set_value(session, app_settings.ANALYSIS_NODES, 1000)

    with pytest.raises(app_settings.SettingsError):
        app_settings.replace(
            session,
            {
                app_settings.ANALYSIS_NODES: 2000,
                app_settings.INACCURACY_THRESHOLD: 40,
                app_settings.MISTAKE_THRESHOLD: 20,
            },
        )

    assert app_settings.get_analysis_nodes(session) == 1000
...
def test_a_put_stores_the_values_and_answers_with_them(api: TestClient) -> None:
    ...
    assert api.get("/api/settings").json()["analysis_multipv"] == 4


def test_a_put_is_the_whole_of_the_settings_rather_than_a_patch(api: TestClient) -> None:
    api.put("/api/settings", json={"analysis_nodes": 50_000, "analysis_multipv": 4})

    assert api.put("/api/settings", json={"analysis_nodes": 60_000}).json() == {
        **UNCONFIGURED,
        "analysis_nodes": 60_000,
    }
...
def test_an_out_of_range_value_is_clamped_rather_than_refused(api: TestClient) -> None:
    ...
    answered = api.put(
        "/api/settings",
        json={"maia_target_elo": 2400, "analysis_multipv": 99, "analysis_nodes": 0},
    ).json()

    assert answered["maia_target_elo"] == MAIA_MAX_RATING
    assert answered["analysis_multipv"] == app_settings.MAX_ANALYSIS_MULTIPV
    assert answered["analysis_nodes"] == app_settings.MIN_NODES
...
def test_a_put_that_names_neither_clears_the_levels(api: TestClient) -> None:
    ...
    assert api.put("/api/settings", json={"analysis_nodes": 1000}).json()["maia_elos"] == [
        MAIA_MAX_RATING
    ]
...
def test_a_run_queued_after_a_put_carries_the_new_budget(
    ...
    api.put("/api/settings", json={"analysis_nodes": 4321, "analysis_multipv": 3})
    ...
    with get_sessionmaker(settings)() as session:
        ...
        # A requested run that names a limit of its own keeps it, and only the line count
        # falls back to the setting.
        requested = analysis.request_analysis(session, game_id=game.id, depth=24)

        assert (imported.nodes, imported.depth, imported.multipv) == (4321, None, 3)
        assert (requested.nodes, requested.depth, requested.multipv) == (None, 24, 3)


def test_a_run_queued_after_a_put_carries_the_maia_pass_it_was_given(
    api: TestClient, settings: Settings
) -> None:
    """Whether Maia runs is settled at enqueue, alongside the budget, and lives on the row."""
    api.put("/api/settings", json={"maia_on_analysis": 0})
    ...
    with get_sessionmaker(settings)() as session:
        ...
        assert (imported.maia, requested.maia) == (False, False)
